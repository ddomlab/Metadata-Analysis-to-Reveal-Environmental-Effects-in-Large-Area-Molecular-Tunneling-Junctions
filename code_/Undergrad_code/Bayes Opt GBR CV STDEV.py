# -*- coding: utf-8 -*-
"""
Created on Sat Jul 20 17:26:33 2024
...
# -*- coding: utf-8 -*-
"""
Created on Tue Jul 16 22:25:55 2024
...
model_df = data[['Humidity', 'Temperature', 'Std Dev', 'Electrode', 'Written Carbon Number', 'Season', 'Time of Day', 'User Type', 'Current Density']].dropna()
...
for feature in numeric_features + categorical_features:
    ...
    plt.boxplot([model_df[model_df[feature] == val]['Std Dev'] for val in model_df[feature].unique()],
                labels=model_df[feature].unique())
    plt.xlabel(feature, fontsize=14, fontweight='bold')
    plt.ylabel('Stdev Log Current Density', fontsize=14, fontweight='bold')
...
# Filter based on Stdev Current Density > 3, there are 3 points with standard deviation
# of 3 with current density above 0 (of 4 points), model overestimation is highly likely here 
...
y_filtered = filtered_results_df['Std Dev']
...
# Re-plot residuals for filtered model
plt.figure(figsize=(12, 8))
plt.scatter(filtered_results_df['Predicted'], filtered_results_df['Residual'], alpha=0.5)
plt.axhline(y=0, color='r', linestyle='--')
plt.xlabel('Predicted Values', fontsize=14, fontweight='bold')
plt.ylabel('Residuals', fontsize=14, fontweight='bold')
plt.title('Residuals vs Predicted Values (Filtered)', fontsize=16, fontweight='bold')
plt.show()

# Re-plot histogram of residuals for filtered model
plt.figure(figsize=(12, 8))
plt.hist(filtered_results_df['Residual'], bins=30, edgecolor='k', alpha=0.7)
plt.xlabel('Residuals', fontsize=14, fontweight='bold')
plt.ylabel('Frequency', fontsize=14, fontweight='bold')
plt.title('Histogram of Residuals (Filtered)', fontsize=16, fontweight='bold')
plt.show()

# Re-plot Q-Q plot for residuals
plt.figure(figsize=(12, 8))
stats.probplot(filtered_results_df['Residual'], dist="norm", plot=plt)
plt.title('Q-Q Plot of Residuals (Filtered)', fontsize=16, fontweight='bold')
plt.show()
...
plt.xlabel('Feature Importance', fontsize=14, fontweight='bold')
plt.ylabel('Feature', fontsize=14, fontweight='bold')
plt.title('Stdev Feature Importance in Gradient Boosting Regressor', fontsize=16, fontweight='bold')
plt.show()
